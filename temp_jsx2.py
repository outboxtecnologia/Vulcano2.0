...
new_func = """export const RecebimentosView = ({ selectedEmpresa }) => {
  ...
  const submitBaixa = async (e, r_id) => {
    ...
    while(nextIdx < flatGrouped.length) {
       ...
       const nxAberto = (!nx.total || nx.total <= 0) && !!nx.num_parcela && nx.num_parcela.toUpperCase() !== 'ATO';
       if (nxAberto) {
           nextR = nx;
           break;
       }
...
print('Updated component RecebimentosView spreadsheet mode successfully.')
