...
export const FiscalSpedView = ({ selectedEmpresa }) => {
    ...
    const BlocoAnalitico = ({ destino, accent }) => {
        ...
        if (loadingAnalitico) return (
            <div className="p-6 text-center text-[10px] uppercase tracking-widest text-[var(--v-text-ghost)] animate-pulse">Carregando conferência por apartamento...</div>
        );
        const grupos = (analiticoData?.grupos || []).filter(g => g.destino === destino);
        if (!grupos.length) return (
            <div className="p-6 text-center text-[10px] uppercase tracking-widest text-[var(--v-text-ghost)]">Nenhum recebimento no destino nesta competência.</div>
        );
        return (
            <div className="border-t border-[var(--v-line)]">
                {grupos.map(g => {
                    ...
                    return (
                        <div key={g.empreendimento} className="mb-2">
                            <div className="px-4 py-2.5 bg-[var(--v-deep)] flex flex-wrap items-center gap-x-6 gap-y-1">
                                <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: accent }}>{g.empreendimento}</span>
                                <span className="text-[9px] font-mono text-[var(--v-text-ghost)]">{g.qtd_unidades} unidade(s) · {g.qtd_parcelas} parcela(s)</span>
                                <span className="text-[9px] font-mono text-[var(--v-text-ghost)]"><span>Parcelas: </span><b className="text-[var(--v-text-bold)]">{fmt(g.valor_parcela)}</b></span>
                                <span className="text-[9px] font-mono text-[var(--v-text-ghost)]"><span>Variação: </span><b className="text-[var(--v-text-bold)]">{fmt(g.variacao)}</b></span>
                                <span className="text-[9px] font-mono text-[var(--v-text-ghost)]"><span>{destino === 'RET_1800' ? 'Base do 1800' : 'Base F200'}: </span><b className="text-[var(--v-text-bold)]">{fmt(g.total_recebido)}</b></span>
                                {destino === 'RET_1800' && (
                                    <span className="text-[9px] font-mono text-[var(--v-text-ghost)]"><span>Guia RET ({g.aliqret}%): </span><b style={{ color: accent }}>{fmt(g.ret_total)}</b></span>
                                )}
                            </div>
                            <table className="w-full text-[10.5px]">
                                ...
                                <tbody>
                                    {rows.map(x => (
                                        <tr key={`${x.venda_id}-${x.destino}`} className="border-b border-[var(--v-line)] hover:bg-[var(--v-hover)] transition-colors">
                                            <td className="px-4 py-2 font-mono font-bold text-[var(--v-text-muted)]">{x.unidade || `VENDA #${x.venda_id}`}</td>
                                            <td className="px-4 py-2 text-[var(--v-text-muted)] max-w-52 truncate">{x.comprador}</td>
                                            <td className="px-4 py-2 font-mono text-[var(--v-text-faint)]">{x.cpf_cnpj}</td>
                                            <td className="px-4 py-2 font-mono text-right text-[var(--v-text-faint)]">{x.qtd_parcelas}</td>
                                            <td className="px-4 py-2 font-mono text-right text-[var(--v-text-muted)]">{fmt(x.valor_parcela)}</td>
                                            <td className="px-4 py-2 font-mono text-right text-[var(--v-text-muted)]">{fmt(x.variacao)}</td>
                                            <td className="px-4 py-2 font-mono text-right text-[var(--v-text-bold)]">{fmt(x.total_recebido)}</td>
                ...
                <p className="px-4 pb-3 text-[9px] text-[var(--v-text-ghost)] uppercase tracking-widest">
                    {destino === 'RET_1800'
            ...
            <div className="bg-[var(--v-deep)] border border-[var(--v-line)] rounded-xl overflow-hidden">
                ...
                {activeTab === 'RET' && retData && (
                    <div>
                        ...
                        {retData.length > 0 && (
                            <>
                                <div className="border-t border-[var(--v-line)] px-4 py-2 flex justify-start">
                                ...
                                <BlocoAnalitico destino="RET_1800" accent="var(--v-accent)" />
                            </>
                        )}
                        {retData.length > 0 && (
                    ...
                    <div>
                        ...
                        {(f200Data.data || []).length > 0 && (
                            <>
                                ...
                                <BlocoAnalitico destino="F200" accent="var(--v-src-questor)" />
                            </>
                        )}
                        {(f200Data.data || []).length > 0 && (
                            <div className="border-t border-[var(--v-line)] p-4 flex justify-end">
