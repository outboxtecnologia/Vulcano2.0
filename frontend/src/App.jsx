...
export default function App() {
  ...
  if (!empresaConfirmed) {
    ...
    return (
      <div className="bg-surface text-on-surface font-body overflow-hidden min-h-screen w-full relative immersive-bg">
    ...
    <div className="bg-[#050505] text-white font-body selection:bg-[#ff4d00]/30 selection:text-white flex h-screen overflow-hidden transition-colors duration-300 immersive-bg">
        <div className="absolute inset-0 bg-black/80 backdrop-blur-[1px] z-0"></div>
        
        {/* Lado Esquerdo - Stitch SideNavBar */}
        <aside className="fixed left-0 top-0 h-full w-64 z-[60] bg-black/40 backdrop-blur-3xl flex flex-col py-6 border-r border-white/5 transition-all">
            <div className="px-6 mb-10 flex items-center gap-3">
                <div className="w-8 h-8 bg-[#ff4d00] rounded-sm flex items-center justify-center shadow-[0_0_15px_#ff4d0066]">
                  <Cpu size={18} className="text-black" />
                </div>
                <div>
                   <h1 className="font-headline font-black text-xl text-white tracking-tighter leading-none">VULCANO</h1>
                   <p className="text-[9px] text-[#ff4d00] tracking-[0.2em] uppercase mt-1 font-black">Construction AI</p>
                </div>
            </div>
            
            <nav className="flex-1 overflow-y-auto w-full no-scrollbar px-3 space-y-8">
                <div className="space-y-1">
                    <h3 className="text-[10px] font-black text-[#555] opacity-60 uppercase tracking-[0.3em] pl-3 mb-3">Core Modules</h3>
                ...
                <div className="space-y-1 border-t border-white/5 pt-6">
                    <h3 className="text-[10px] font-black text-[#555] opacity-60 uppercase tracking-[0.3em] pl-3 mb-3">Enterprise Suite</h3>
            ...
            <div className="px-4 mt-auto">
                <button className="w-full bg-[#ff4d00] text-black py-4 rounded-sm font-black text-[10px] uppercase tracking-[0.3em] flex items-center justify-center hover:bg-white transition-all shadow-[0_0_20px_#ff4d0033]">
        ...
        <div className="ml-64 flex-1 flex flex-col h-full bg-transparent transition-colors relative z-10 w-full overflow-hidden">
            ...
            <header className="h-16 border-b border-white/5 flex items-center justify-between px-8 bg-black/20 backdrop-blur-2xl shrink-0 z-50 transition-colors">
                <div className="flex items-center gap-4 flex-1">
                    <div className="relative group">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#555] group-focus-within:text-[#ff4d00] transition-colors" size={16} />
                        <input 
                            className="bg-white/5 border border-white/5 text-xs rounded-sm pl-10 pr-4 py-2 w-96 focus:border-[#ff4d00]/30 text-white outline-none transition-all placeholder:text-[#444]" 
                ...
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-2 bg-black/40 px-4 py-2 rounded-sm border border-white/5 shadow-2xl transition-colors hover:border-[#ff4d00]/30">
                        <Database size={12} className="text-[#ff4d00] shrink-0" />
                        <select 
                            value={selectedEmpresa}
                            onChange={(e) => setSelectedEmpresa(e.target.value)}
                            className="bg-transparent border-none outline-none text-[10px] w-52 text-white font-black uppercase tracking-[0.2em] cursor-pointer appearance-none truncate"
                        >
                            {globalEmpresas.map(emp => (
                                <option key={emp.id} value={emp.id} className="bg-[#111] text-white normal-case font-medium">
                                    {emp.id} - {emp.nome}
                                </option>
                            ))}
                        </select>
                    ...
                    <div className="flex items-center gap-4 h-full">
                        ...
                        <button onClick={toggleTheme} className="p-2 text-[#555] hover:text-[#ff4d00] transition-all bg-white/5 border border-white/5 rounded-sm">
                            {theme === 'dark' ? <Sun size={14} /> : <Moon size={14} />}
                        </button>
                        <button onClick={handleRunSQL} className="text-[#555] hover:text-[#ff4d00] transition-all relative"><Download size={18}/></button>
                        <button className="text-[#555] hover:text-[#ff4d00] transition-all relative">
                            <Bell size={18} />
            ...
            {/* MAIN CONTENT AREA */}
            <main className="flex-1 overflow-x-hidden overflow-y-auto bg-transparent transition-colors p-8 relative custom-scrollbar">
              <div className="absolute top-0 right-0 w-[1000px] h-[1000px] bg-[#ff4d00]/10 rounded-full blur-[150px] pointer-events-none -translate-y-1/2 translate-x-1/3"></div>
