          ...
          <div className="p-4 border-b border-[var(--v-border)] flex items-center justify-between bg-black/20">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={handleGravarNoErp} disabled={gravandoErp} className="flex items-center gap-1.5 px-4 py-1.5 bg-[#3dd68c]/10 text-[#3dd68c] border border-[#3dd68c]/30 rounded text-[11px] font-bold hover:bg-[#3dd68c]/20 transition-colors shadow-sm disabled:opacity-40">
                <Save size={14} /> {gravandoErp ? 'Gravando...' : (targetTable === 'VENDAS' ? `Importar ${matchData.filter(r => r.status?.startsWith('PRONTA')).length} venda(s)` : targetTable === 'RECEBIMENTOS' ? `Baixar ${matchData.filter(r => r.status === 'MATCH_PERFEITO').length} parcela(s)` : 'Gravar no ERP')}
              </button>
            </div>
          </div>
