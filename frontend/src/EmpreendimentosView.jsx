    ...
    <div className="space-y-1">
      ...
      <div className="relative">
        <input
          ...
          className="w-full bg-[var(--v-bg)] border border-[var(--v-border)] p-2.5 text-xs text-[var(--v-text-bold)] outline-none focus:border-[var(--v-accent)]/50 transition-all font-mono"
        />
        ...
        {aberto && filtradas.length > 0 && (
          <div className="absolute z-50 left-0 right-0 mt-1 max-h-52 overflow-y-auto rounded border border-[var(--v-border)] shadow-2xl custom-scrollbar" style={{ background: 'var(--v-card)' }}>
            {filtradas.slice(0, 40).map(c => (
              <button type="button" key={c.id} disabled={c.tipo === 1}
                ...
                className={`w-full text-left px-2.5 py-1.5 text-[10.5px] ${c.tipo === 1 ? 'opacity-40 cursor-default' : 'hover:bg-[var(--v-tint)] cursor-pointer'}`}>
