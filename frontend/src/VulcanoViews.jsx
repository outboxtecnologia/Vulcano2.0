    ...
    <div className="flex flex-col h-full animate-in fade-in" style={{ background: 'var(--v-shell)' }}>
                                    ...
                                    return (
                                        ...
                                        >
                                            <input type="checkbox" checked={selecao.has(v.id)}
                                                ...
                                                className="accent-[var(--v-err)] cursor-pointer" />
                                            ...
                                            <button
                                                ...
                                                title="Editar venda"
                                                className="p-1 rounded opacity-40 group-hover/row:opacity-100 hover:bg-[var(--v-tint)] transition-all"
                                                style={{ color: 'var(--v-warn)' }}>
                                                <Pencil size={13}/>
                                            </button>
                                        </div>
                                    );
        ...
        {selectedVenda && (
            <div className="w-[450px] flex flex-col shrink-0 animate-in slide-in-from-right-8 duration-300 z-10" style={{ background: 'var(--v-shell)', borderLeft: '1px solid var(--v-line-warm)' }}>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="p-5" style={{ borderBottom: '1px solid var(--v-line-warm)' }}>
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <div className="text-[15px] font-semibold" style={{ color: 'var(--v-text-bold)' }}>{condicoesLoading ? '...' : (parcelas.length ? parcelas.length + 'x' : '—')}</div>
                            </div>
                            <div className="mt-2">
                                <div className="font-mono text-[9.5px] tracking-[0.22em] mb-1" style={{ color: 'var(--v-text-faint)' }}>ENTRADA (SINAL/ATO)</div>
                                <div className="text-[15px] font-semibold" style={{ color: 'var(--v-text-bold)' }}>{condicoesLoading ? '...' : (entradaForma ? formatCurrency(entradaForma.valor) : '—')}</div>
                            </div>
                            <div className="mt-2">
                                <div className="font-mono text-[9.5px] tracking-[0.22em] mb-1" style={{ color: 'var(--v-text-faint)' }}>RECEBIDO</div>
                                <div className="text-[15px] font-semibold" style={{ color: 'var(--v-ok)' }}>{condicoesLoading ? '...' : formatCurrency(recebido)}</div>
                            </div>
                        </div>
                        {!condicoesLoading && !formas.length && !parcelas.length && (
                            <p className="mt-3 text-[9.5px] uppercase tracking-widest" style={{ color: 'var(--v-text-muted)' }}>Venda sem condições de pagamento cadastradas — use "Lançar parcela manual" ou recadastre com condições.</p>
                        )}
                    </div>
                        );
                    })()}
                    ...
                    <div className="p-5" style={{ borderBottom: '1px solid var(--v-line-warm)' }}>
                        <div className="font-mono text-[9.5px] tracking-[0.22em] mb-3" style={{ color: 'var(--v-text-faint)' }}>CRONOGRAMA · PRÓXIMAS 12</div>
                        <div className="h-12 flex items-end justify-center">
                            {condicoesLoading ? (
                                ...
                                <span className="text-[9px] uppercase" style={{ color: 'var(--v-text-faint)' }}>Sem parcelas em aberto</span>
                            )}
                        </div>
                    </div>
                    ...
                    {estruturaAberta && (
                        <div className="p-5" style={{ borderBottom: '1px solid var(--v-line-warm)' }}>
                            <div className="font-mono text-[9.5px] tracking-[0.22em] mb-3" style={{ color: 'var(--v-text-faint)' }}>ESTRUTURA FINANCEIRA · {(condicoesData?.parcelas || []).length} PARCELAS</div>
                            <div className="max-h-56 overflow-y-auto custom-scrollbar">
                                <table className="w-full text-[10.5px]">
                                    <tbody>
                                        {(condicoesData?.parcelas || []).map(p => (
                                            <tr key={p.id} className="border-b" style={{ borderColor: 'var(--v-line-warm)' }}>
                                                <td className="py-1 pr-2 font-mono" style={{ color: 'var(--v-text-muted)' }}>{p.parcela || '—'}</td>
                                                <td className="py-1 pr-2 font-mono" style={{ color: 'var(--v-text-muted)' }}>{p.data}</td>
                                                <td className="py-1 pr-2 font-mono text-right" style={{ color: 'var(--v-text-bold)' }}>{formatCurrency(p.valor_parcela)}</td>
                                                <td className="py-1 font-mono text-right" style={{ color: p.total_pago > 0 ? 'var(--v-ok)' : 'var(--v-text-faint)' }}>{p.total_pago > 0 ? formatCurrency(p.total_pago) : 'aberta'}</td>
                                            </tr>
                                        ))}
                                        {!(condicoesData?.parcelas || []).length && (
                                            <tr><td className="py-2 text-[10px] uppercase" style={{ color: 'var(--v-text-faint)' }}>Sem parcelas — lance manualmente abaixo.</td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
                    ...
                    <div className="p-5">
                        ...
                        <div className="flex flex-col gap-1.5">
                            <button onClick={() => setEstruturaAberta(v => !v)} className="flex justify-between items-center px-4 py-2.5 rounded-lg text-[12px] font-medium transition-colors hover:bg-[var(--v-tint)]" style={{ color: 'var(--v-text-bold)' }}>
                                <span className="flex items-center gap-3"><Layers size={14} style={{ color: 'var(--v-text-muted)' }}/> <span>{estruturaAberta ? 'Ocultar' : 'Abrir'} estrutura financeira</span></span>
                            </button>
                            <button onClick={() => setParcelaManualModal({ venda: selectedVenda, data: hojeLocal(), valor: '', referencia: '', tipo: 'AVULSA', quantidade: 1, intervalo_meses: 6 })} className="flex justify-between items-center px-4 py-2.5 rounded-lg text-[12px] font-medium transition-colors hover:bg-[var(--v-tint)]" style={{ color: 'var(--v-text-bold)' }}>
                                <span className="flex items-center gap-3"><DollarSign size={14} style={{ color: 'var(--v-text-muted)' }}/> Lançar parcela manual</span>
                            </button>
                            <button onClick={() => abrirEdicaoVenda(selectedVenda)} className="flex justify-between items-center px-4 py-2.5 rounded-lg text-[12px] font-medium transition-colors hover:bg-[var(--v-tint)]" style={{ color: 'var(--v-text-bold)' }}>
                                <span className="flex items-center gap-3"><Pencil size={14} style={{ color: 'var(--v-text-muted)' }}/> Editar venda</span>
                            </button>
                            <button onClick={() => prepararExclusao([selectedVenda.id])} title="Exclusão definitiva do registro (com prévia) — para registro errado/teste; distrato é outra ação" className="flex justify-between items-center px-4 py-2.5 rounded-lg text-[12px] font-medium transition-colors hover:bg-red-950/30" style={{ color: 'var(--v-err)' }}>
      ...
      {parcelaManualModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[110] animate-in fade-in p-6">
            <div className="w-full max-w-md rounded-xl shadow-2xl flex flex-col p-6" style={{ background: 'var(--v-bg)', border: '1px solid var(--v-line-warm)' }}>
                <h3 className="text-lg font-black uppercase tracking-widest flex items-center gap-3 mb-1" style={{ color: 'var(--v-text-bold)' }}>
                    <DollarSign size={20} color="var(--v-accent)"/> <span>Lançar Parcela Manual</span>
                </h3>
                <p className="text-[11px] mb-5" style={{ color: 'var(--v-text-muted)' }}>Venda <b style={{ color: 'var(--v-text-bold)' }}>#{parcelaManualModal.venda.id}</b> — {parcelaManualModal.venda.cliente_nome}. As parcelas entram como previstas (em aberto) e podem ser baixadas normalmente.</p>
                <div className="grid grid-cols-3 gap-3 mb-3">
                    <div>
                        <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>Tipo (formato de condição)</label>
                        <select value={parcelaManualModal.tipo || 'AVULSA'} onChange={(e) => setParcelaManualModal({ ...parcelaManualModal, tipo: e.target.value, quantidade: ['MENSAL','SEMESTRAL','ANUAL','REFORCO'].includes(e.target.value) ? (parcelaManualModal.quantidade > 1 ? parcelaManualModal.quantidade : 2) : 1 })} className="w-full p-2.5 rounded text-[11px] outline-none" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }}>
                    ...
                    <div>
                        <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>Quantidade</label>
                    ...
                    {parcelaManualModal.tipo === 'REFORCO' ? (
                        <div>
                            <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>Intervalo (meses)</label>
                            <input type="number" min="1" max="60" value={parcelaManualModal.intervalo_meses || 6} onChange={(e) => setParcelaManualModal({ ...parcelaManualModal, intervalo_meses: e.target.value })} className="w-full p-2.5 rounded text-[11px] outline-none" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }} />
                        </div>
                    ) : (
                        <div>
                            <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>Referência (avulsa)</label>
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                    <div>
                        <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>1º vencimento</label>
                        <input type="date" min="1990-01-01" value={parcelaManualModal.data} onChange={(e) => setParcelaManualModal({ ...parcelaManualModal, data: e.target.value })} className="w-full p-2.5 rounded text-[11px] outline-none dark-calendar" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }} />
                    </div>
                    <div>
                        <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>Valor por parcela</label>
                        <input type="number" step="0.01" value={parcelaManualModal.valor} onChange={(e) => setParcelaManualModal({ ...parcelaManualModal, valor: e.target.value })} placeholder="0,00" className="w-full p-2.5 rounded text-[11px] outline-none" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }} />
                    </div>
                </div>
                {(() => {
                    ...
                    return (
                        <p className="text-[10.5px] mb-4" style={{ color: 'var(--v-warn)' }}>
                ...
                <div className="flex justify-end gap-3">
                    <button onClick={() => setParcelaManualModal(null)} className="px-4 py-2 hover:bg-[var(--v-tint)] transition-colors text-[11px] font-bold uppercase tracking-widest rounded" style={{ color: 'var(--v-text-muted)' }}>Cancelar</button>
                    <button onClick={salvarParcelaManual} disabled={salvandoParcela} className="px-6 py-2 rounded text-[11px] font-bold uppercase tracking-widest disabled:opacity-50" style={{ background: 'linear-gradient(135deg, var(--v-accent), var(--v-accent-2))', color: '#1a0a04' }}>
      ...
      {excluirModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[120] animate-in fade-in p-6">
            <div className="w-full max-w-lg rounded-xl shadow-2xl flex flex-col p-6" style={{ background: 'var(--v-bg)', border: '1px solid rgba(239, 68, 68, 0.35)' }}>
                ...
                <p className="text-[11px] mb-4" style={{ color: 'var(--v-text-muted)' }}>
                ...
                <div className="rounded p-3 mb-3 text-[11px] space-y-1" style={{ background: 'rgba(239, 68, 68, 0.06)', border: '1px solid rgba(239, 68, 68, 0.2)', color: 'var(--v-text-bold)' }}>
                    <p className="font-black uppercase text-[10px]" style={{ color: 'var(--v-text-muted)' }}>O que será removido</p>
                ...
                {(excluirModal.preview.bloqueadas || []).length > 0 && (
                    <div className="rounded p-3 mb-3 text-[10.5px] space-y-1" style={{ background: 'rgba(255, 149, 0, 0.08)', border: '1px solid rgba(255, 149, 0, 0.3)', color: 'var(--v-text-bold)' }}>
                        <p className="font-black uppercase text-[var(--v-warn)]">{excluirModal.preview.bloqueadas.length} venda(s) com pagamento — fora da exclusão por padrão:</p>
                        ...
                        <label className="flex items-center gap-2 mt-2 cursor-pointer" style={{ color: 'var(--v-warn)' }}>
                            <input type="checkbox" checked={excluirModal.forcar} onChange={(e) => setExcluirModal({ ...excluirModal, forcar: e.target.checked })} className="accent-[var(--v-warn)]" />
                            Incluir também as vendas com pagamento (as baixas locais serão removidas)
                        </label>
                    </div>
                )}
                <div className="flex justify-end gap-3">
                    <button onClick={() => setExcluirModal(null)} className="px-4 py-2 hover:bg-[var(--v-tint)] transition-colors text-[11px] font-bold uppercase tracking-widest rounded" style={{ color: 'var(--v-text-muted)' }}>Cancelar</button>
      ...
      {editVendaModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[110] animate-in fade-in p-6">
            <div className="w-full max-w-md rounded-xl shadow-2xl flex flex-col p-6" style={{ background: 'var(--v-bg)', border: '1px solid var(--v-line-warm)' }}>
                <h3 className="text-lg font-black uppercase tracking-widest flex items-center gap-3 mb-1" style={{ color: 'var(--v-text-bold)' }}>
                    <Pencil size={18} color="var(--v-accent)"/> <span>Editar Venda</span>
                </h3>
                <p className="text-[11px] mb-5" style={{ color: 'var(--v-text-muted)' }}>
                    Venda <b style={{ color: 'var(--v-text-bold)' }}>#{editVendaModal.venda.id}</b> — {editVendaModal.venda.cliente_nome}.
                    {(editVendaModal.venda.qtd_compradores || 1) > 1 && (
                        <> Contrato com <b style={{ color: 'var(--v-warn)' }}>{editVendaModal.venda.qtd_compradores} compradores</b>: alterar o valor total redistribui as cotas na mesma proporção.</>
                    )}
                </p>
                <div className="grid grid-cols-2 gap-3 mb-4">
                    <div>
                        <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>Data da venda</label>
                        <input type="date" min="1990-01-01" value={editVendaModal.data} onChange={(e) => setEditVendaModal({ ...editVendaModal, data: e.target.value })} className="w-full p-2.5 rounded text-[11px] outline-none dark-calendar" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }} />
                    </div>
                    <div>
                        <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>Valor total do contrato</label>
                        <input type="number" step="0.01" value={editVendaModal.total} onChange={(e) => setEditVendaModal({ ...editVendaModal, total: e.target.value })} placeholder="0,00" className="w-full p-2.5 rounded text-[11px] outline-none" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }} />
                    </div>
                    <div>
                        <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>Nº contrato</label>
                        <input value={editVendaModal.num_contrato} onChange={(e) => setEditVendaModal({ ...editVendaModal, num_contrato: e.target.value })} placeholder="default: id da venda" className="w-full p-2.5 rounded text-[11px] outline-none font-mono" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }} />
                    </div>
                    <div>
                        <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>Permuta</label>
                        <select value={editVendaModal.permuta} onChange={(e) => setEditVendaModal({ ...editVendaModal, permuta: e.target.value })} className="w-full p-2.5 rounded text-[11px] outline-none" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }}>
                            <option value="N">Não</option>
                            <option value="S">Sim</option>
                        </select>
                    </div>
                ...
                <div className="flex justify-end gap-3">
                    <button onClick={() => setEditVendaModal(null)} className="px-4 py-2 hover:bg-[var(--v-tint)] transition-colors text-[11px] font-bold uppercase tracking-widest rounded" style={{ color: 'var(--v-text-muted)' }}>Cancelar</button>
                    <button onClick={salvarEdicaoVenda} disabled={salvandoEdicao} className="px-6 py-2 rounded text-[11px] font-bold uppercase tracking-widest disabled:opacity-50" style={{ background: 'linear-gradient(135deg, var(--v-accent), var(--v-accent-2))', color: '#1a0a04' }}>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                    <div>
                        ...
                        <input type="date" value={distratoModal.data_distrato_form || hojeLocal()}
                            ...
                            className="w-full p-2.5 rounded text-[11px] outline-none dark-calendar" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }} />
                    </div>
                    <div>
                        ...
                        <input type="number" step="0.01" value={distratoModal.valor_devolvido_form ?? ''} placeholder="0,00"
                            ...
                            className="w-full p-2.5 rounded text-[11px] outline-none" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }} />
                    </div>
                </div>
                <div className="flex justify-end gap-3">
    ...
    <div className="flex flex-col h-full animate-in fade-in" style={{ background: 'var(--v-shell)' }}>
      <div className="px-6 py-4 flex flex-col gap-4 shrink-0 z-20" style={{ borderBottom: '1px solid var(--v-line-warm)' }}>
        ...
        <div className="flex flex-wrap gap-3 items-center">
            ...
            <div className="flex items-center gap-1 px-2 py-1 rounded-lg" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)' }}>
                <button onClick={() => navegarCompetencia(-1)} title="Mês anterior" className="p-1.5 rounded hover:bg-[var(--v-tint)] transition-colors" style={{ color: 'var(--v-accent)' }}><ChevronLeft size={14}/></button>
                <button onClick={() => navegarCompetencia(0, true)} title="Ir para o mês atual" className="min-w-[64px] text-center font-mono text-[12px] font-bold px-1 hover:text-[var(--v-accent)] transition-colors" style={{ color: 'var(--v-text-bold)' }}>{competenciaLabel}</button>
                <button onClick={() => navegarCompetencia(1)} title="Próximo mês" className="p-1.5 rounded hover:bg-[var(--v-tint)] transition-colors" style={{ color: 'var(--v-accent)' }}><ChevronRight size={14}/></button>
            </div>
            ...
            {loading && data.length === 0 ? (
                ...
                <div className="flex flex-col">
                    {Object.entries(grouped).filter(([_, items]) => items.length > 0).map(([groupName, items]) => (
                        <div key={groupName} className="mb-4">
                            <div className="px-6 py-2 flex items-center gap-3 sticky top-0 z-10" style={{ background: 'var(--v-shell)' }}>
                                <span className={`font-mono text-[9.5px] font-bold tracking-[0.28em] ${groupName === 'VENCIDAS' ? 'text-red-500' : groupName === 'VENCE HOJE' ? 'text-orange-500' : 'text-[var(--v-text-faint)]'}`}>
                                    {groupName}
                                </span>
                                <div className="flex-1 h-[1px]" style={{ background: 'var(--v-line-warm)' }}></div>
                                <span className="font-mono text-[9.5px]" style={{ color: 'var(--v-text-faint)' }}>{items.length}</span>
                                    ...
                                    return (
                                        <div id={`row-${r.id}`} key={r.id} className="flex flex-col" style={{ borderBottom: '1px solid var(--v-line-warm)' }}>
                                            ...
                                            >
                                                ...
                                                <div className={`font-medium text-[13.5px] font-mono ${isVencida ? 'text-red-400' : 'text-[var(--v-text-bold)]'}`}>{formatCurrency(r.parcela)}</div>
      ...
      {editParcelaModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[110] animate-in fade-in p-6">
            <div className="w-full max-w-md rounded-xl shadow-2xl flex flex-col p-6" style={{ background: 'var(--v-bg)', border: '1px solid var(--v-line-warm)' }}>
                <h3 className="text-lg font-black uppercase tracking-widest flex items-center gap-3 mb-1" style={{ color: 'var(--v-text-bold)' }}>
                    <Pencil size={18} color="var(--v-accent)"/> <span>Editar Parcela</span>
                </h3>
                <p className="text-[11px] mb-5" style={{ color: 'var(--v-text-muted)' }}>
                    <b style={{ color: 'var(--v-text-bold)' }}>#{String(editParcelaModal.row.id).replace('prazo_', 'pr_')}</b> — {editParcelaModal.row.cliente} · {editParcelaModal.row.descricao_venda}
                    {String(editParcelaModal.row.id).startsWith('prazo_') && <> <span style={{ color: 'var(--v-warn)' }}>(parcela de cronograma ainda não efetivada)</span></>}
                </p>
                <div className="grid grid-cols-2 gap-3 mb-4">
                    <div>
                        <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>Vencimento</label>
                        <input type="date" min="1990-01-01" value={editParcelaModal.data} onChange={(e) => setEditParcelaModal({ ...editParcelaModal, data: e.target.value })} className="w-full p-2.5 rounded text-[11px] outline-none dark-calendar" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }} />
                    </div>
                    <div>
                        <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>Valor da parcela</label>
                        <input type="number" step="0.01" value={editParcelaModal.valor} onChange={(e) => setEditParcelaModal({ ...editParcelaModal, valor: e.target.value })} placeholder="0,00" className="w-full p-2.5 rounded text-[11px] outline-none" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }} />
                    </div>
                    <div>
                        <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>Parcela (rótulo)</label>
                        <input value={editParcelaModal.rotulo} onChange={(e) => setEditParcelaModal({ ...editParcelaModal, rotulo: e.target.value })} placeholder="ex.: 95/240" className="w-full p-2.5 rounded text-[11px] outline-none font-mono" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }} />
                    </div>
                    {!String(editParcelaModal.row.id).startsWith('prazo_') && (
                        <div>
                            <label className="text-[10px] uppercase font-bold mb-1 block" style={{ color: 'var(--v-text-muted)' }}>Observação</label>
                            <input value={editParcelaModal.obs} onChange={(e) => setEditParcelaModal({ ...editParcelaModal, obs: e.target.value })} className="w-full p-2.5 rounded text-[11px] outline-none" style={{ background: 'var(--v-card)', border: '1px solid var(--v-line-warm)', color: 'var(--v-text-bold)' }} />
                        </div>
                    )}
                </div>
                <div className="flex justify-end gap-3">
                    <button onClick={() => setEditParcelaModal(null)} className="px-4 py-2 hover:bg-[var(--v-tint)] transition-colors text-[11px] font-bold uppercase tracking-widest rounded" style={{ color: 'var(--v-text-muted)' }}>Cancelar</button>
                    <button onClick={salvarEdicaoParcela} disabled={salvandoParcelaEdit} className="px-6 py-2 rounded text-[11px] font-bold uppercase tracking-widest disabled:opacity-50" style={{ background: 'linear-gradient(135deg, var(--v-accent), var(--v-accent-2))', color: '#1a0a04' }}>
