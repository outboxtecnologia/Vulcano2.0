...
export const RecebimentosView = ({ selectedEmpresa }) => {
  ...
  const uniqueClientes = [...new Set(filteredBase.map(r => r.cliente_nome).filter(Boolean))].sort();
  ...
  const totalPago = filtered.reduce((acc, curr) => acc + ((curr.total_pago > 0) ? curr.total_pago : 0), 0);
  const totalParcela = filtered.reduce((acc, curr) => acc + (curr.valor_parcela || 0), 0);
  const totalVariacao = filtered.reduce((acc, curr) => acc + (curr.valor_variacao || 0), 0);
                     ...
                     {paginatedData.map((r, idx) => {
                       ...
                       return (
                         <tr key={idx} className={`border-b border-[var(--v-border)] transition-colors hover:bg-[var(--v-hover)] ${isAtrasada ? 'bg-[var(--v-accent)]/5 border-l-2 border-l-[var(--v-accent)]' : ''}`}>
                            ...
                            <td className="p-3 text-right font-black text-[var(--v-accent)] text-[13px]">{!isAberto ? formatCurrency(r.total_pago) : isAtrasada ? <span className="text-[var(--v-accent)] text-[9px] uppercase px-2 py-0.5 bg-[var(--v-accent)]/10 rounded">Atrasada</span> : <span className="text-[var(--v-text-faint)] text-[9px] uppercase">A Vencer</span>}</td>
                            <td className="p-3 text-right font-bold text-[var(--v-accent-3)] font-mono">{formatCurrency(r.valor_parcela)}</td>
                            <td className="p-3 text-right font-bold text-[var(--v-accent-6)] font-mono">{formatCurrency(r.valor_variacao)}</td>
                            <td className="p-3 text-[var(--v-text-muted)] truncate max-w-[150px]">{r.descricao_venda}</td>
                            <td className="p-3 text-[var(--v-text)] truncate max-w-[150px] font-bold">{r.cliente_nome}</td>
                 ...
                 <tbody>
                   ...
                   {filtered.slice(0, 1000).map((r, i) => (
                     <tr key={i} className="border-b border-[var(--v-border)] hover:bg-[var(--v-hover)] text-[var(--v-text)] font-mono">
                       <td className="p-3 border-r border-[var(--v-border)] text-[var(--v-text-muted)]">{r.cliente_nome_cnpj || '-'}</td>
                       <td className="p-3 border-r border-[var(--v-border)] font-sans truncate max-w-[200px]">{r.cliente_nome}</td>
                       ...
                       <td className="p-3 border-r border-[var(--v-border)] text-right text-[var(--v-accent-3)] bg-[var(--v-accent-3)]/5">{formatCurrency(r.valor_parcela)}</td>
                       <td className="p-3 border-r border-[var(--v-border)] text-right text-[var(--v-text-red)]">{r.desconto > 0 ? formatCurrency(r.desconto) : '-'}</td>
                       <td className="p-3 border-r border-[var(--v-border)] text-right text-[var(--v-accent-6)] font-semibold">{r.valor_variacao > 0 ? formatCurrency(r.valor_variacao) : '-'}</td>
                       <td className="p-3 border-r border-[var(--v-border)] text-right text-[var(--v-accent)] font-bold bg-[var(--v-accent)]/10">{formatCurrency(r.total_pago)}</td>
