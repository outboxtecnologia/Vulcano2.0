...
function TabelaMapaComparativa({ questor, vulcano1, vulcano2, dashboardMeta, empresaId, filtroEmpNome, nomesEmpresasAuditadas }) {
  ...
  // ── Modal RACIONAL do APTO ────────────────────────────────────────────────
  const racionalModal = detalheApto ? (() => {
    const rawAptoNum = detalheApto.replace(/\D/g, '');
    const todasVendas = [];
    // filtroEmpNome aqui já é o nome exato do empreendimento desta conta
    // (derivado pelo ContaConfronto via empreendimento_nome da conta virtual)
    // 🔍 DEBUG — remover após confirmar correto
    console.log('[RACIONAL] filtroEmpNome (derived):', filtroEmpNome, '| dashboardMeta keys:', Object.keys(dashboardMeta || {}));

    Object.entries(dashboardMeta || {}).forEach(([empNomeKey, emp]) => {
      // Match exato ou substring — ambos os nomes vêm da EMPREENDIMENTO.NOME do Vulcano
