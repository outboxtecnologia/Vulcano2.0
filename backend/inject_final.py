...
new_functions = r'''class PreviewBaixasInput(BaseModel):
    empresa_id: int
        ...
        for row in data.extracted_data:
            ...
            for grupo_key, grupo_vendas in grupos_unidades.items():
                ...
                for cand in grupo_vendas:
                    ...
                    cur.execute("""
                        SELECT p.ID, p.DATA, p.VALOR, vfp.ID
                        FROM VENDAFORMAPAGTOPRAZO p
                        JOIN VENDAFORMAPAGTO vfp ON vfp.ID = p.IDVENDAFORMAPAGTO
                        WHERE vfp.IDVENDA = ? AND (p.VALOR_PAGO IS NULL OR p.VALOR_PAGO = 0)
                    """, (v_id,))
                    for pr in cur.fetchall(): 
                        if pr[0] not in reserved_ids_prazos: # IMPEDE DUPLICIDADE NO ARQUIVO E IGNORA ZUMBIS PAGOS
                    ...
                    for combo_tamanho in [2, 3, 4]:
                        ...
                        for combo in combinations(pool_abertas, combo_tamanho):
                            ...
                            if soma_combo > 0 and soma_combo <= total_pago:
                                ...
                                # Diferenca toleravel de acréscimo 100% num agrupador consolidado
                                if diff_rate < 1.0: 
            ...
            if melhor_match_final:
                if 'MULTIPLO' in melhor_match_final['type']:
                    ...
                    for idx, item in enumerate(lista):
                        ...
                        calc_acresc = float(row.get('acrescimos_variacoes', 0) or 0) / len(lista)
                        if total_div > float(p_valor or 0) and calc_acresc == 0:
                            calc_acresc = total_div - float(p_valor or 0)
                        ...
                        results.append({
                            ...
                            'proposta_ia': {'novo_total_pago': total_div, 'novo_desconto': float(row.get('descontos', 0) or 0) / len(lista), 'novo_acrescimo': calc_acresc, 'projetada': False}
                        })
                else:    
...
start_kw = 'class PreviewBaixasInput(BaseModel):'
end_kw = '@app.post("/api/fiscal/f200")'

s_idx = text.find(start_kw)
e_idx = text.find(end_kw)

if s_idx != -1 and e_idx != -1:
    new_text = text[:s_idx] + new_functions + '\n# --- ROTAS BASE: ÁREA FISCAL E SERO ---\n' + text[e_idx:]
