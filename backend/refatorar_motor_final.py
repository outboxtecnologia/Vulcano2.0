...
new_func = r'''@app.post("/api/parser/preview-baixas")
    ...
    try:
        ...
        for row in data.extracted_data:
            ...
            if not unidade and not comprador_nome:
                # Fallback to pure CPF if available
                pass
            ...
            for grupo_key, grupo_vendas in grupos_unidades.items():
                ...
                if match_perfeito:
                    ...
                    mat_reason = f"Match Diamante Global ({mat_type})" if cand['is_diamante'] else f"Match Ouro Global ({mat_type})"
...
for i, line in enumerate(lines):
    if '@app.post("/api/parser/preview-baixas")' in line:
        start_idx = i
    if 'class CommitBaixasInput(BaseModel):' in line:
        end_idx = i
        break
...
if start_idx != -1 and end_idx != -1:
    ...
    with open('main.py', 'w', encoding='utf-8') as f:
        f.write(new_script)
    print("SUCCESS: REPLACED ENTIRE PREVIEW_BAIXAS ENDPOINT safely!")
else:
