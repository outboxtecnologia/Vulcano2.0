...
new_logic = r'''            # Pesquisa Rápida em RAM (In-Memory Filter) com Score de Vendas
            ...
            for grupo_key, grupo_vendas in grupos_unidades.items():
                ...
                # 1. Tenta Match Singular Rápido (Receber ou Prazo)
                ...
                # 2. Tenta Combinação Múltipla (Ex: Pagou 2 ou 3 boletos juntos) no Receber
                if not match_perfeito and len(pool_abertas) >= 2:
                    ...
                    for combo_tamanho in [2, 3, 4]:
                        ...
                        for combo in combinations(pool_abertas, combo_tamanho):
                            ...
                            if abs(soma_combo - total_pago) < 10.0:
                                lista_multipla = list(combo)
                                mat_type = "MULTIPLO_RECEBER"
                                achou_combo = True
                                break
                                
                # 3. Tenta Correção CUB Margem (Receber e Projeção) SE FOI DIAMANTE
                if not match_perfeito and not lista_multipla:
                    ...
                    if diamante_no_grupo:
                        for p_tuple in pool_abertas:
                            ...
                            if p_valor > 0:
                                if total_pago > p_valor and (abs(total_pago - p_valor) / p_valor) < 1.0:
                                    match_perfeito = p_tuple
                        ...
                        if not match_perfeito:
                            for pr_tuple in pool_prazos:
                                ...
                                if pr_valor > 0:
                                    if total_pago > pr_valor and (abs(total_pago - pr_valor) / pr_valor) < 1.0:
                                        match_perfeito = pr_tuple
            ...
            if melhor_match_final:
                if melhor_match_final['type'] == 'MULTIPLO_RECEBER':
                    ...
                    for idx, item in enumerate(lista):
                        ...
                        if idx > 0:
                            nova_row['cpf_cnpj'] += f" [Rateio {idx+1}/{len(lista)}]"
    ...
    try:
        ...
        for i, line in enumerate(lines):
            if 'Pesquisa Rápida em RAM (In-Memory Filter)' in line:
                start_idx = i
            if 'else:' in line and 'Falha Matemática' in lines[i+1] or 'else:' in line and 'base_fail_result' in lines[i+1] or 'PROJETADA_NOVA_LINHA_NATIVA' in line:
                pass
        
        # We need a robust end_idx logic:
        # Looking down from start_idx, find the first line that matches the end of the main loop.
        # "results.append(base_fail_result)" in the else block.
        # It's better to just track bracket/intentation.
