...
if start_idx != -1:
    # Find the FIRST occurrence of the end_str *after* start_idx
    # Wait, the poc block ends with `           )}`.
    # Let's find the occurrence of `           )}` that is BEFORE `Area Removida: Llama Painel`
    
    end_marker = "{/* Area Removida: Llama Painel */}"
    end_marker_idx = app_code.find(end_marker)
    
    # The end of the block is the `)}` right before end_marker
    slice_before_marker = app_code[start_idx:end_marker_idx]
    
    # We find the last `)}` inside this slice
