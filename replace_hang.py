...
if match:
    text = text[:match.start()] + new_code + "\n    " + text[match.end()-32:] 
    # taking care not to remove # 4. Fallback Clássico 1:1 Fuzzy
    # Wait, the match.end() includes # 4. Fallback Clássico 1:1 Fuzzy. So we need to put it back.
    # Ah, new_code already includes # 4. Fallback Clássico 1:1 Fuzzy!
