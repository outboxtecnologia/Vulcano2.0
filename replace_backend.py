...
import re
# Find the bounds: @app.post("/api/auditoria/concilia-orfaos") down to "matches":              deduped,\n    }
pattern = r'@app\.post\("/api/auditoria/concilia-orfaos"\)[\s\S]*?    }'
match = re.search(pattern, text)
if not match:
    print("Could not find Target API")
    exit(1)
...
async def api_concilia_orfaos(data: ConciliaOrfaosInput):
    ...
    for i, q in enumerate(data.orfaos_questor):
        d = q.dict()
        d["_id"] = f"Q_{i}"
        d["_cluster"] = _extrair_clusters(d.get("historico", "") + " " + d.get("logica", ""))
        d["_usado"] = False
        q_items.append(d)
    ...
    for q in qs_restantes:
        ...
        encontrou = False
        limit_r = min(6, len(vs_restantes) + 1)
    ...
    for v in vs_restantes2:
        ...
        encontrou = False
        limit_rq = min(6, len([q for q in q_items if not q["_usado"]]) + 1)
    ...
    for m in matches_finais:
        if "questor_detalhe" in m:
            for d in m["questor_detalhe"]: d.pop("_id", None); d.pop("_cluster", None); d.pop("_usado", None)
            for d in m["vulcano_detalhe"]: d.pop("_id", None); d.pop("_cluster", None); d.pop("_usado", None)
    ...
    return {
        ...
        "matches": matches_finais,  # Front end vai tratar o payload inteiro
    }'''
